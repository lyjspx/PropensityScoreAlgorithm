...
hist.mirror2 <- function(data.plot, label){
  myplot <- ggplot(data.plot, aes(ps, fill=status))+
    geom_histogram(data=subset(data.plot, status=="Yes"), aes(ps, y=..count..))+
...
weight.test <- function(data,form, molecular.pri, is.continuous=TRUE, weight="MW",
                        mirror.plot=FALSE,cancer, data.type, outdir=".",
                        perm=FALSE, seed=seed){  
  ...
  #write.table(t(rbind(rownames(clinical.common),ifelse(clinical.common$Z==1, "FEMALE","MALE"))),
  #            file=paste(cancer,"_",data.type,"_samples.txt", sep=""),quote = F,row.names = F,sep="\t")
  ...
  if(mirror.plot)
  {
    ...
    pdf(paste(outdir,"/",cancer,"_", data.type,
              ifelse(perm,paste("_perm_",seed, sep=""), ""),
              "_mirror_raw.pdf",
              sep=""))
    if(analysis=="omt")
    {
  ...
  for (i in 2:(ncol(clinical.common)-1))
  {
    ...
    if(nrow(clinical.common[which(clinical.common[,i]==1),]) >=3){
      ...
      std.diff <- check.balance(index.tr, 
                                index.ctr, clinical.common[,i],
                                wt, printout=TRUE)
      if(std.diff>cutoff)
      {
  ...
  for (i in 1:ncol(molecular.common))
  {
    ...
    # get weighted mean
    molecular.0.w <- c(molecular.0.w,
                       sum(as.numeric(molecular.common[index.ctr,i])*wt[index.ctr],na.rm=T)/
                         sum(wt[index.ctr],na.rm=T))
    molecular.1.w <- c(molecular.1.w,
                       sum(as.numeric(molecular.common[index.tr,i])*wt[index.tr],na.rm=T)/
                         sum(wt[index.tr],na.rm=T))
    #print(i)
    if (i%%1000==0)
    {
      print(i)
    ...
    if(is.continuous)
    {
      ...
      pvalue <- summary(glm(as.numeric(molecular.common[,i])~clinical.common$Z, family=binomial, weights=wt),
                        na.rm=T)$coef[2,4]
      coef <- summary(glm(as.numeric(molecular.common[,i])~clinical.common$Z, family=binomial, weights=wt),
                      na.rm=T)$coef[2,1]
      if (class(pvalue)=="try-error")
      {
        pvalue <- NA
  ...
  return(list(feature=colnames(molecular.common),
              coef=molecular.coefs, pvalue=molecular.pvalues,fdr=molecular.fdr ,
...
summarize.p <- function(molecular.data, molecular.result, cutoff=0.05, print=FALSE )
{
  ...
  if(print)
  {
    print(cbind(colnames(molecular.data)[signif.index],signif(pvalue[signif.index],3),
                signif(fdr[signif.index],3), signif(coef[signif.index],3),
                signif(mean.0[signif.index],3), signif(mean.1[signif.index],3)))
  }
  return(list(feature.sig=colnames(molecular.data)[signif.index],
              pvalue.sig=pvalue[signif.index], 
              fdr.sig= fdr[signif.index], n.sig=length(signif.index),
              coef.sig=coef[signif.index], mean0.sig=mean.0[signif.index],
              mean1.sig=mean.1[signif.index], mean0.sig.w=mean.0.w[signif.index], 
              mean1.sig.w=mean.1.w[signif.index]))#, n.sig=length(which(molecular.result$pvalue<cutoff))) )
...
write.summary <- function(summary, topic, analysis, type)
{
  file <- paste(topic,"_",analysis,"_summary_",type,".txt", sep="")
  data <- data.frame(summary$feature.sig, summary$fdr.sig,
                     summary$coef.sig, summary$mean0.sig,
                     summary$mean1.sig, summary$mean0.sig.w, summary$mean1.sig.w)
  colnames(data) <- c("feature","fdr","coef","mean_trt1",
                      "mean_trt2", "mean_trt1_weighted", "mean_trt2_weighted")
  write.table(data,file=file, quote=FALSE, sep="\t", col.names=TRUE, row.names=FALSE)
}

write.result <- function(result, topic, analysis, type, fdr.cutoff=0.05)
{
  file <- paste(topic,"_",analysis,"_result_",type,".txt", sep="")
...
plot.perm <- function(perm, n, topic, analysis, type, cutoff)
{        
  ...
  file <- paste(topic,"_",analysis,"_perm_",type,"_",cutoff,".pdf", sep="")
...
perm.cal <- function(topic, analysis, type, molecular.pri, cutoff=0.05, seedV=1:100)
{
  print(paste("Calculate permutation for", topic,":",type,":"))
  #  load(file=paste(topic,"_", analysis,"_result.RData",sep=""))
  ...
  for (seed in seedV)
  {
    ...
    if(type=="mut")
    {
      load(paste(scripts.dir, "/",topic,"_",analysis,"/perm_sig_result_",seed,".RData", sep=""))
    }else if(type=="pre")
    {
      load(paste(scripts.dir, "/",topic,"_",analysis,"/perm_sig_result_pre_",seed,".RData", sep=""))
    }else{
  ...
  ##print (perm)
  ##print (n)
  plot.perm(perm, n, topic, analysis, type, cutoff)
}
